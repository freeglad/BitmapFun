...
//    private static final String TAG = "ImageCache";
	...
	public static int DEFAULT_MEM_CACHE_SIZE = 1024 * 5; // 5MB

    // Default disk cache size in bytes
	public static int DEFAULT_DISK_CACHE_SIZE = 1024 * 1024 * 50; // 50MB

    // Compression settings when writing images to disk cache
    private static final CompressFormat DEFAULT_COMPRESS_FORMAT = CompressFormat.JPEG;
    public static int DEFAULT_COMPRESS_QUALITY = 90;
    private static final int DISK_CACHE_INDEX = 0;

    // Constants to easily toggle various caches
    public static boolean DEFAULT_MEM_CACHE_ENABLED = true;
    public static boolean DEFAULT_DISK_CACHE_ENABLED = true;
    public static boolean DEFAULT_INIT_DISK_CACHE_ON_CREATE = false;

    private DiskLruCache mDiskLruCache;
//    private LruCache<String, BitmapDrawable> mMemoryCache;
    private LruCache<String, WeakReference<BitmapDrawable>> mMemoryCache;
    private ImageCacheParams mCacheParams;
    private final Object mDiskCacheLock = new Object();
    private boolean mDiskCacheStarting = true;

//    private HashSet<SoftReference<Bitmap>> mReusableBitmaps;
    private HashSet<WeakReference<Bitmap>> mReusableBitmaps;
    ...
    private void init(ImageCacheParams cacheParams) {
        ...
        if (mCacheParams.memoryCacheEnabled) {
            if (ImageFetcher.isDebug) {
                Log.d(ImageFetcher.TAG, "ImageCache Memory cache created (size = " + mCacheParams.memCacheSize + ")");
            }
            ...
            mMemoryCache = new LruCache<String, WeakReference<BitmapDrawable>>(mCacheParams.memCacheSize) {
                ...
                protected void entryRemoved(boolean evicted, String key,
                		WeakReference<BitmapDrawable> oldValue, 
...
//                    }
                ...
                if (mCacheParams.diskCacheEnabled && diskCacheDir != null) {
                    ...
                    if (getUsableSpace(diskCacheDir) > mCacheParams.diskCacheSize) {
                        try {
                            ...
                            if (ImageFetcher.isDebug) {
                                Log.d(ImageFetcher.TAG, "ImageCache Disk cache initialized");
                            }
                        } catch (final IOException e) {
                            mCacheParams.diskCacheDir = null;
                            if(ImageFetcher.isDebug)
                            	Log.d(ImageFetcher.TAG, "ImageCache initDiskCache - " + e);
                        }
                    }
                }
            }
    ...
    public BitmapDrawable getBitmapFromMemCache(String data) {
        ...
        if (mMemoryCache != null) {
            WeakReference<BitmapDrawable> weakValue = mMemoryCache.get(data);
            if(weakValue != null)
            	memValue = weakValue.get();
        }

        if (ImageFetcher.isDebug && memValue != null) {
            Log.d(ImageFetcher.TAG, "ImageCache Memory cache hit");
        }
    ...
    public Bitmap getBitmapFromDiskCache(String data) {
        ...
        Bitmap bitmap = null;
        if (ImageFetcher.isDebug) {
            Log.d(ImageFetcher.TAG, "ImageCache Disk cache key=" + key + "; data=" + data);
        }
        synchronized (mDiskCacheLock) {
            ...
            if (ImageFetcher.isDebug) {
                Log.d(ImageFetcher.TAG, "ImageCache Disk cache after mDiskCacheLock");
            }
            if (mDiskLruCache != null) {
            	if (ImageFetcher.isDebug) {
                    Log.d(ImageFetcher.TAG, "ImageCache Disk cache mDiskLruCache is not null");
                }
                ...
                try {
                    ...
                    if (snapshot != null) {
                        if (ImageFetcher.isDebug) {
                            Log.d(ImageFetcher.TAG, "ImageCache Disk cache hit");
                        }
                        inputStream = snapshot.getInputStream(DISK_CACHE_INDEX);
                        if (inputStream != null) {
                        	if (ImageFetcher.isDebug) {
                                Log.d(ImageFetcher.TAG, "ImageCache Disk cache inputStream is not null");
                            }
...
//                                    fd, Integer.MAX_VALUE, Integer.MAX_VALUE, this);
                            ...
                            bitmap = ImageResizer.decodeSampledBitmapFromDescriptor(
                                    fd, 
                                    mImageWidth > 0 ? mImageWidth : Integer.MAX_VALUE, 
                                    mImageHeight > 0 ? mImageHeight : Integer.MAX_VALUE, 
                                    this);
                        }
                    }
                } catch (final Exception e) {
                    if(ImageFetcher.isDebug)
                		Log.d(ImageFetcher.TAG, "ImageCache getBitmapFromDiskCache - " + e);
                } finally {
                    try {
                        if (inputStream != null) {
                            inputStream.close();
    ...
    public void clearMemoryCache()
    {
      if (this.mMemoryCache != null) {
        try {
        	...
        	if (ImageFetcher.isDebug) {
            	Log.d(ImageFetcher.TAG, e.getMessage());
          	}
        }

        if (ImageFetcher.isDebug) {
        	Log.d(ImageFetcher.TAG, "Memory cache cleared");
      	}
      }
    }
    ...
    public void clearCache() {
        if (mMemoryCache != null) {
            ...
            if (ImageFetcher.isDebug) {
                Log.d(ImageFetcher.TAG, "ImageCache Memory cache cleared");
            }
        }
        ...
        synchronized (mDiskCacheLock) {
            ...
            if (mDiskLruCache != null && !mDiskLruCache.isClosed()) {
                try {
                    ...
                    if (ImageFetcher.isDebug) {
                        Log.d(ImageFetcher.TAG, "ImageCache Disk cache cleared");
                    }
                } catch (IOException e) {
                    if(ImageFetcher.isDebug)
                    	Log.d(ImageFetcher.TAG, "ImageCache clearCache - " + e);
                }
                mDiskLruCache = null;
                initDiskCache();
            }
        ...
        synchronized (mDiskCacheLock) {
            if (mDiskLruCache != null) {
                try {
                    mDiskLruCache.flush();
                    if (ImageFetcher.isDebug) {
                        Log.d(ImageFetcher.TAG, "ImageCache Disk cache flushed");
                    }
                } catch (IOException e) {
                    if(ImageFetcher.isDebug)
                		Log.d(ImageFetcher.TAG, "ImageCache flush - " + e);
                }
            }
        }
    }
            ...
            if (mDiskLruCache != null) {
                try {
                    if (!mDiskLruCache.isClosed()) {
                        ...
                        mDiskLruCache = null;
                        if (ImageFetcher.isDebug) {
                            Log.d(ImageFetcher.TAG, "ImageCache Disk cache closed");
                        }
                    }
                } catch (IOException e) {
                    if(ImageFetcher.isDebug)	
                    	Log.d(ImageFetcher.TAG, "ImageCache close - " + e);
                }
            }
        }
    }
    ...
    public static File getDiskCacheDir(Context context, String uniqueName) {
    	...
    	if(ImageFetcher.isDebug) {
    		...
    		Log.d(ImageFetcher.TAG, "ExternalDir is null ? " + (f1 == null) + "; CacheDir is null ? " + (f2 == null));
    	}
